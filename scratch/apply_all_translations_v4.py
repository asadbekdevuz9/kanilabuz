...
# Brand text description in Column 1 (Fixed regex to prevent greedy multi-line matches)
...
# Services list title
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Xizmatlar\'.*?\}.*?\{\s*cart\.length\s*\}.*?\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'ta\'.*?\}', re.DOTALL),
