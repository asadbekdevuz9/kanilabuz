...
# Reset App.tsx to clean state
subprocess.run(["git", "checkout", "src/App.tsx"])
print("Git checkout src/App.tsx completed successfully.")
...
# --- 2. Typed.js Hero strings (Keep DOTALL) ---
...
# --- 3. Form Validation errors (Keep DOTALL) ---
...
# --- 6. Footer columns & bottom legal links (No DOTALL for single line matches) ---
col2_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bosh Ofis & Ish Tartibi\'.*?\}')
...
tcity_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Termiz Shahri\'.*?\}')
...
worktime_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Ish Vaqti\'.*?\}')
...
col3_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Tezkor Bog.*?\}')
...
privacy_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Maxfiylik Siyosati\'.*?\}')
...
terms_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Foydalanish shartlari\'.*?\}')
...
gallery_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Fotogalereya\'.*?\}')
...
certs_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Sertifikatlarimiz\'.*?\}')
...
livestats_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Online Statistika\'.*?\}')
...
brand_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'KANI-LAB\s*-\s*eng\s*ilg.*?or.*?tarmog.*?i\.\'.*?\}')
...
# --- 7. Hero Section CTAs ---
onlayn_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Onlayn navbat olish\'.*?\}')
...
tahlillar_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Tahlillar va narxlar\'.*?\}')
...
about_dd_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Kani-Lab haqida\'.*?\}')
...
news_links_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Yangiliklar\'.*?\}')
content = sub_pat(news_links_pattern, "{getLangTextInline('Yangiliklar', 'Новости', 'Haberler', 'News')}", content, "Yangiliklar link")

muloqot_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'MULOQOT TARMOQLARI\'.*?\}')
...
hist_title_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bizning tariximiz\'.*?\}')
...
hist_sub_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Yillar davomida ishonchli rivojlanish.*?\}')
...
val_title_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bizning qadriyatlarimiz\'.*?\}')
...
val_sub_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Sifat va ishonch poydevori\'.*?\}')
...
# Values Cards Array (Keep DOTALL)
...
mission_title_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bizning maqsad va vazifalarimiz\'.*?\}')
...
mission_sub_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Kelajakka yo.*?naltirilgan maqsadlar\'.*?\}')
...
# Tasks block
tasks_header_pattern = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Asosiy vazifalarimiz:\'.*?\}')
...
h_priv_title = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'MAXFIYLIK SIYOSATI\'.*?\}')
...
h_priv_sub = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Kani-Lab laboratoriyasi ma.*?lumotlar maxfiyligi siyosati\'.*?\}')
...
if idx_priv != -1:
    ...
    print("13. Privacy activeTab block NOT found!")

# --- 14. Terms of Use Headers ---
h_terms_title = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'FOYDALANISH SHARTLARI\'.*?\}')
...
h_terms_sub = re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Kani-Lab veb-platformasidan foydalanish bo.*?yicha shartlar va qoidalar\'.*?\}')
...
if idx_terms != -1:
    ...
    print("15. Terms activeTab block NOT found!")

# --- 16. Booking Confirmation Preview ---
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'PREMIUM DIAGNOSTIKA MARKAZI\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bemor\'.*?\}'),
    "{getLangTextInline('Bemor', 'Пациент', 'Hasta', 'Patient')}",
    content, "Bemor"
)

content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Chek\'.*?\}'),
    "{getLangTextInline('Chek', 'Чек', 'Bilet/Fatura', 'Receipt/Ticket')}",
    content, "Chek"
)

content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'JAMI SUMMA\'.*?\}'),
    "{getLangTextInline('JAMI SUMMA', 'ИТОГОВАЯ СУММА', 'TOPLAM TUTAR', 'TOTAL AMOUNT')}",
    content, "JAMI SUMMA"
)

content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Xizmatlar\'.*?\}.*?\{\s*cart\.length\s*\}.*?\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'ta\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*label\s*:\s*lang\s*===\s*\'uz\'\s*\?\s*\'Sana\'.*?\}'),
    "{ label: getLangTextInline('Sana', 'Дата', 'Tarih', 'Date'), value: selectedDate }",
    content, "Sana Label"
)
content = sub_pat(
    re.compile(r'\{\s*label\s*:\s*lang\s*===\s*\'uz\'\s*\?\s*\'Vaqt\'.*?\}'),
    "{ label: getLangTextInline('Vaqt', 'Время', 'Saat', 'Time'), value: selectedTime }",
    content, "Vaqt Label"
)
content = sub_pat(
    re.compile(r'\{\s*label\s*:\s*lang\s*===\s*\'uz\'\s*\?\s*\'Filial\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*["\']Sog.*?lig.*?ingiz.*?maqsadimiz["\'].*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Yordam va qo.*?llab-quvvatlash\'.*?\}'),
    "{getLangTextInline('Yordam va qo‘llab-quvvatlash', 'Помощь и поддержка', 'Yardım ve Destek', 'Help & Support')}",
    content, "Help & Support"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Muammo yuzaga kelsa, biz bilan bog.*?laning\'.*?\}'),
...
# --- 17. Analyzer Popup ---
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*selectedAnalyzer\.category\.uz\s*:\s*selectedAnalyzer\.category\.ru\s*\}'),
    "{getLangText(selectedAnalyzer.category)}",
    content, "analyzer category"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*selectedAnalyzer\.shortDesc\.uz\s*:\s*selectedAnalyzer\.shortDesc\.ru\s*\}'),
    "{getLangText(selectedAnalyzer.shortDesc)}",
    content, "analyzer shortDesc"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Qisqacha tavsif\'.*?\}'),
    "{getLangTextInline('Qisqacha tavsif', 'Краткое описание', 'Kısa Açıklama', 'Brief Description')}",
    content, "Qisqacha tavsif"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Ilmiy va texnik ma.*?lumotlar\'.*?\}'),
...
# --- 18. Real-time Live Analytics ---
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'KaniLab Live Platforma Statistikasi\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Real vaqt rejimida tashrif buyuruvchilar faolligi va geolokatsiyasi\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'foydalanuvchi faol\'.*?\}'),
    "{getLangTextInline('foydalanuvchi faol', 'активных пользователей', 'aktif kullanıcı', 'users active')}",
    content, "users active count"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Hozircha faol foydalanuvchilar yo.*?q\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Fikr qoldirish\'.*?\}'),
    "{getLangTextInline('Fikr qoldirish', 'Оставить отзыв', 'Yorum Bırak', 'Leave a Review')}",
    content, "Fikr qoldirish"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Xizmatlarimiz haqida o\\\'z fikringizni yozib qoldiring\.\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Ism va familiya\'.*?\}'),
    "{getLangTextInline('Ism va familiya', 'Имя и фамилия', 'İsim ve Soyisim', 'First and Last Name')}",
    content, "Ism va familiya"
)
content = sub_pat(
    re.compile(r'placeholder=\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Ismingizni kiriting\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Mijoz turi\'.*?\}'),
    "{getLangTextInline('Mijoz turi', 'Тип клиента', 'Müşteri Türü', 'Client Type')}",
    content, "Mijoz turi"
)
content = sub_pat(
    re.compile(r'<\s*option\s*value\s*=\s*""\s*>\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Tanlang\.\.\.\'.*?\}\s*<\s*/\s*option\s*>'),
    "<option value=\"\">{getLangTextInline('Tanlang...', 'Выберите...', 'Seçiniz...', 'Select...')}</option>",
    content, "Tanlang option"
)
content = sub_pat(
    re.compile(r'<\s*option\s*value\s*=\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bemor\'.*?\}\s*>\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bemor\'.*?\}\s*<\s*/\s*option\s*>'),
...
content = sub_pat(
    re.compile(r'<\s*option\s*value\s*=\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Shifokor\'.*?\}\s*>\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Shifokor\'.*?\}\s*<\s*/\s*option\s*>'),
...
content = sub_pat(
    re.compile(r'<\s*option\s*value\s*=\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Hamkor\'.*?\}\s*>\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Hamkor\'.*?\}\s*<\s*/\s*option\s*>'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Fikringiz\'.*?\}'),
    "{getLangTextInline('Fikringiz', 'Ваш отзыв', 'Görüşünüz', 'Your Review')}",
    content, "Fikringiz"
)
content = sub_pat(
    re.compile(r'placeholder=\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Taassurotlaringiz bilan o\\\'rtoqlashing\.\.\.\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Yuborish\'.*?\}'),
...
# --- 20. News Section ---
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'YANGILIKLAR\'.*?\}'),
    "{getLangTextInline('YANGILIKLAR', 'НОВОСТИ', 'HABERLER', 'NEWS')}",
    content, "YANGILIKLAR title"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Kani-Lab yangiliklari va videolari\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*item\.category\s*===\s*\'video\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'FOTOGALEREYA\'.*?\}'),
    "{getLangTextInline('FOTOGALEREYA', 'ФОТОГАЛЕРЕЯ', 'FOTO GALERİ', 'PHOTO GALLERY')}",
    content, "FOTOGALEREYA title"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Kani-Lab fotogalereyasi\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Barchasi\'.*?\}'),
    "{getLangTextInline('Barchasi', 'Все', 'Hepsi', 'All')}",
    content, "Barchasi filter"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Ilmiy hamkorlik \(TerDU\)\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Qon olish shahobchasi \(2022\)\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'8-mart bayrami\'.*?\}'),
    "{getLangTextInline('8-mart bayrami', 'Праздник 8 марта', '8 Mart Bayramı', \"International Women's Day\")}",
    content, "8-mart filter"
)
content = sub_pat(
    re.compile(r'\?\s*\(\s*lang\s*===\s*\'uz\'\s*\?\s*\'TerDU hamkorligi\'.*?\)\s*:\s*\(\s*lang\s*===\s*\'uz\'\s*\?\s*\'Qon olish shahobchasi\'.*?\)'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bizning Sertifikatlar\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*cert\.year\s*\}\s*\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Yil\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Qaysi filial sizga yaqin\?\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Uydan namuna olish xizmati ham mavjud\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Bog\\\'lanish\'.*?\}'),
    "{getLangTextInline('Bog\\\'lanish', 'Связаться', 'İletişim', 'Contact')}",
    content, "Contact button"
)
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Landmark:\s*Sobiq Ko.*?z kasalxonasi.*?\'.*?\}'),
...
content = sub_pat(
    re.compile(r'\{\s*lang\s*===\s*\'uz\'\s*\?\s*\'Ariza qabul qilindi!\'.*?\}'),
...
with open(filepath, "w", encoding="utf-8") as f:
    f.write(content)
print("All replacements written back to src/App.tsx successfully.")
